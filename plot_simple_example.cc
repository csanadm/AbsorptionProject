...
int main()
{
  //Setting up ROOT TCanvas to draw on
  TCanvas *c = new TCanvas("c","c",1024,768);
  gStyle->SetOptStat(0);
  c->SetGrid(1,1);
  //Setting up frame histogram, only for title and axes manipulation
  ...
  frame->Draw();
  //Air absorption, original formula
  ...
  //Finalizing canvas and saving graphics
  c->SetLogx(1);
  c->SetLogy(1);
  c->Modified();
  ...
  frame->SetMaximum(10);
  frame->Draw();
  //Water absorption, original formula
  ...
  frame->SetMaximum(2);
  frame->Draw();
  //Construction material absorption, original data points
  ...
  //Construction material absorption, approximation
  ...
  //Deleting ROOT objects to clean memory
  delete frame;
