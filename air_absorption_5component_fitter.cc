...
int main(int argc, char *argv[])
{
  ...
  ROOT::Math::Functor ftr(&FuncToMin,13); 
  ...
  double flim4 = fiOC*pow(cC/ciOC*fiOC*fiOC/fC/fC,1.0/(nOC-2));
  ...
  min.SetLimitedVariable(     8, "n5",    n5 ,     0.1,  0.0, 2.5);
  min.SetLimitedVariable(     9, "flim1", flim1,   1.0,  flim1/5, flim1*5);
  min.SetLimitedVariable(     10, "flim2", flim2,   5.0,  flim2/5, flim2*5);
  min.SetLimitedVariable(     11, "flim3", flim3,   10.,  flim3/5, flim3*5);
  min.SetLimitedVariable(     12, "flim4", flim4,   10.,  flim4/5, flim4*5); 
  ...
  min.FixVariable(8); //n5
  min.FixVariable(9); //flim1
  min.FixVariable(10); //flim2
  min.FixVariable(11); //flim3
  min.FixVariable(12); //flim4
  if(flim1<20) min.FixVariable(9);
  if(flim3>20000) min.FixVariable(11);
  if(flim4>20000) min.FixVariable(12);
...
*/
  ...
  for(int ipar=0;ipar<13;ipar++) cerr << min.VariableName(ipar) << "\t" << min.X()[ipar] << "\t +- \t" << min.Errors()[ipar] << endl;
  ...
  TF1* alphafunc = new TF1("alphafunc",AFunc,frmin,frmax,13);
  alphafunc->SetParameters(min.X());
  alphafunc->SetLineColor(8);
  alphafunc->SetLineWidth(2);
  alphafunc->Draw("SAME");
  
  TF1* approxfunc = new TF1("approxfunc",Approx,frmin,frmax,13);
  ...
  frame->Draw();
  TF1* approxdiff = new TF1("approxdiff",ApproxDiff,frmin,frmax,13);
 ...
 /*
  ofstream outfile("air_absorption_5component_fitter.new.out",ofstream::app);
