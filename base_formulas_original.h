...
double h(double hrel, double T, double P)
{
  return hrel*psat(T)/P;
}

double Nit(double T, double h, double f, double P)
{
  double frn = FrN(T,h,P);
  return  868.6*0.1068*exp(-3352/T)*pow(T/T0,-5./2.)*frn * f*f/(frn*frn + f*f);
}

double Oxyg(double T, double h, double f, double P)
{
  double  fro = FrO(h,P);
  return  868.6*0.01275*exp(-2239.1/T)*pow(T/T0,-5./2.)*fro * f*f/(fro*fro+f*f);
}

double AlphaAir(double T, double h, double f, double P)
{
  return classical(T,f) + Nit(T,h,f,P) + Oxyg(T,h,f,P);
}
...
double NFuncAir(const double *x, const double *pars)
{
  double f = x[0];
  double TKelvin = pars[0] + T01;  //convert from celsius to kelvin
  double H       = pars[1];        //Relative humidity
  double P       = pars[2] * Patm; //convert from atmosphere to pascal
  
  return Nit(TKelvin, h(H,TKelvin,P), f, P);
}
...
double OFuncAir(const double *x, const double *pars)
{
  ...
  return Oxyg(TKelvin, h(H,TKelvin,P), f, P);
}
